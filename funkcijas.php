...
function array_sql_insert($aFields,$mtable,$db){
    $insert_str="INSERT INTO ".$mtable." SET ";
    $k=0;
    $aGar=count($aFields);
    foreach ($aFields as $OneField){
        $fld=$OneField[$k];
        $k = $k + 1;
        $insert_str=$insert_str.$fld.'=:'.$fld;
        if ($k<$aGar) {
    ...
    $q = $db->prepare($insert_str);

    $k=0;
    $array_string='$data = array(';
    foreach ($aFields as $OneField){
        $fld=$OneField[$k];
        $array_string=$array_string."':".$fld."' => $".$fld;
        if ($k<$aGar) {
